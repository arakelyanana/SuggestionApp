namespace SuggestionAppUI.Pages
{
    public partial class AdminApproval
    {
        ...
        private async Task ApproveSubmission(SuggestionModel submision)
        {
            submision.ApproveForRelease = true;
            submissions.Remove(submision);
            await suggestionData.UpdateSuggestion(submision);
        }

        private async Task RejectSubmission(SuggestionModel submision)
        {
            submision.Rejected = true;
            submissions.Remove(submision);
            await suggestionData.UpdateSuggestion(submision);
        }
