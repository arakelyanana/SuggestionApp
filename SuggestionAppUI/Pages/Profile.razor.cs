    ...
    public partial class Profile
    {
        ...
        protected async override Task OnInitializedAsync()
        {
            ...
            if (loggedInUser is not null && results is not null)
            {
                ...
                approved = submissions.Where(s => s.ApproveForRelease && s.Archived == false && s.Rejected == false).ToList();
                archived = submissions.Where(s => s.Archived && s.Rejected == false).ToList();
                pending = submissions.Where(s => s.ApproveForRelease == false && s.Rejected == false).ToList();
